...
const timeline = [
  { year: '2022', title: 'Started the Journey', desc: 'HTML, CSS, JavaScript — built first weather app and fell in love with web development.' },
  { year: '2023', title: 'React & Full Stack', desc: 'Dived into React, Node.js, MongoDB, PostgreSQL. Built first production full-stack apps.' },
  { year: '2023', title: 'First Hackathon Win', desc: 'Smart India Hackathon — led a 5-person team to build an AI attendance system. Won college level.' },
  { year: '2024', title: 'AI & Advanced Stack', desc: 'LangChain, RAG chatbots, Supabase, TypeScript. Started freelancing for real clients.' },
  { year: '2025', title: 'Building & Growing', desc: 'Open source contributions, SaaS projects, developer community leadership.' },
];
    ...
    <section id="about" ref={ref} className="relative py-28 bg-white overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-20">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={fd(0.15)} className="space-y-5">
            ...
            <p className="text-base text-stone leading-relaxed">
              ...
              backend with <span className="text-ink font-semibold">Node.js</span>, and integrating
              <span className="text-orange-DEFAULT font-semibold"> AI/ML capabilities</span> — from RAG chatbots to real-time dashboards.
            </p>
            ...
            {/* Strength bars */}
            <div className="card p-5 space-y-3 mt-4">
              <h3 className="font-heading font-semibold text-sm text-ink mb-1">Core Strengths</h3>
              {[
                ...
                <div key={name}>
                  <div className="flex justify-between text-xs mb-1"><span className="font-mono text-stone">{name}</span><span className="text-orange-DEFAULT font-mono">{pct}%</span></div>
          ...
          <motion.div ref={statsRef} initial={{ opacity: 0, x: 20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={fd(0.3)} className="space-y-5">
            {/* 3D floating shapes */}
            <div className="h-[240px] rounded-2xl overflow-hidden bg-cream border border-black/6">
              <FloatingShapes />
            </div>
            {/* Stats grid */}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={fd(0.5)}>
          ...
          <div className="relative pl-6 border-l-2 border-black/8 space-y-6">
            {timeline.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -16 }} animate={inView ? { opacity: 1, x: 0 } : {}}
                transition={{ delay: 0.6 + i * 0.1 }} className="relative">
                <div className="absolute -left-[29px] top-5 w-3.5 h-3.5 rounded-full border-2 border-orange-DEFAULT bg-white" />
                <motion.div whileHover={{ x: 4 }} transition={{ duration: 0.2 }} className="card p-5 hover:shadow-card-hover transition-all">
